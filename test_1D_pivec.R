...
write.csv(output, file='1Dtest_pivec.csv')
...
#figure
pdf("1Dtest_pivec.pdf")
par(mfrow=c(1, 2), mgp=c(2, 0.5, 0), mar=c(3, 3, 2, 1)+0.1)
matplot(pi0.vec, fdr1.mthd, type="o", pch=1:5, lwd=2, main="FDR Comparison", 
        xlab=expression(pi[paste(1,d)]), ylab="FDR", ylim=c(0.01, 0.30))
legend("topleft", c("LASS.or","LASS.dd","LAWS.or", "LAWS.dd","BH"), pch=1:5, col=1:5, lwd=2)
matplot(pi0.vec, etp1.mthd, type="o", pch=1:5, lwd=2, main="Power Comparison", 
        xlab=expression(pi[paste(1,d)]), ylab="Power",ylim=c(0.01, 1.00))
legend("topleft", c("LASS.or","LASS.dd","LAWS.or", "LAWS.dd",'BH'), pch=1:5, col=1:5, lwd=2)
dev.off()
