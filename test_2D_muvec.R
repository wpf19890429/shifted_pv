...
# 2D spatial domain with n1*n2 points
...
chol_factor<-Matrix::Cholesky(Matrix::crossprod(D) + Matrix::Diagonal(m),
                              perm=TRUE, super=FALSE) #Computes the Cholesky decomposition of a sparse
...
valuelist=list('n','m','np','mu0.vec','q','locs')
...
for_out1 <- foreach(iter = 1:(np*nrep), .combine='rbind') %dopar%
{
  ...
  mu0<-mu0.vec[i]
  set.seed(iter+2000) #Random seeds are set to reproduce the results
  for(i1 in 1:n)
  {
    for (j1 in 1:n)
    {
  ...
  while(abs(ks-kold)>1e-4 && passcounter<=100) {
    ...
    fl0 = list(x = rep(mean(x.vec), m), # likelihood term
               ...
               u_dual = rep(0,length(x.vec)), # scaled dual variable for constraint x = z
...
fdr1<-mean_out1[,4:8]
etp1<-mean_out1[,9:13]
write.csv(for_out1, file='2Dtest_muvec.csv')

pdf("2Dtest_muvec.pdf")

par(mfrow=c(1, 2), mgp=c(2, 0.5, 0), mar=c(3, 3, 2, 1)+0.1)

matplot(mu0.vec, fdr1, type="o", pch=1:5, lwd=2, main="FDR Comparison", xlab=expression(mu), ylab="FDR", ylim=c(0.01, 0.30))
legend("topleft", c("LASS.or","LASS.dd","LAWS.or", "LAWS.dd","BH"), pch=1:5, col=1:5, lwd=2)

matplot(mu0.vec, etp1, type="o", pch=1:5, lwd=2, main="Power Comparison", xlab=expression(mu), ylab="Power",ylim=c(0.01, 1.00))
legend("topleft", c("LASS.or","LASS.dd","LAWS.or", "LAWS.dd","BH"), pch=1:5, col=1:5, lwd=2)
